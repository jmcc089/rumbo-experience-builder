 ...
 * Read-only, no login (consistent with the rest of Rumbo).
 *
 * Margin/markup figures ARE shown here — this is the operator's own surface,
 * separate from the client and provider routes.
 */
export default async function OperatorPage() {
  const [metrics, requests, providerPanel, repairOrders] = await Promise.all([
    getDashboardMetrics(),
    getRecentRequests(),
    getProviderResponsePanel(),
    getOrdersForRepair(),
    ...
    <div className={styles.shell}>
      <header className={styles.topbar}>
        <div className={styles.topbarInner}>
          ...
          <span className={styles.portalTag}>Operator dashboard</span>
        </div>
      </header>
      ...
      <main className={styles.main}>
        ...
        <RepairPanel orders={repairOrders} />
      </main>
    </div>
  );
}
