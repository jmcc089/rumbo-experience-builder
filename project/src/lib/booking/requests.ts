...
async function buildOrderSummary(itinerary: ItinerarySnapshot): Promise<OrderSummary> {
  ...
  const expName = new Map<string, string>(expRes.rows.map((r: any) => [r.id, r.name]));
  const lodgingName = new Map<string, string>(lodgingRes.rows.map((r: any) => [r.id, r.name]));
  const zoneName = new Map<string, string>(zoneRes.rows.map((r: any) => [r.id, r.name]));
...
async function buildOrderItems(itinerary: ItinerarySnapshot, travelers: number): Promise<NewOrderItem[]> {
  ...
  const expNetPrice = new Map<string, number>(expRes.rows.map((r: any) => [r.id, Number(r.net_price)]));
  const lodgingNetPrice = new Map<string, number>(
    lodgingRes.rows.map((r: any) => [r.id, Number(r.net_price_per_night)])
  );
