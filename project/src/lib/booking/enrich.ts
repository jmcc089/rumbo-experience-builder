...
async function loadCatalog(snapshots: ItinerarySnapshot[]): Promise<CatalogMaps> {
  ...
  return {
    ...
    lodging: new Map(lodgingRes.rows.map((r: any) => [r.id, { name: r.name, tier: r.tier as LodgingTier }])),
    zone: new Map(zoneRes.rows.map((r: any) => [r.id, r.name])),
  };
}
