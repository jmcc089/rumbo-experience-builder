...
async function main() {
  ...
  const intake = {
    ...
    budget_total: 2000,
    prefs_json: {
      interests: ["nature", "culture"] as any,
  ...
  const view2 = await getProposals(token);
  ...
  const anyNetPrice = itemsRow.rows.some((r: any) => payResultStr.includes(`"net_price":${r.net_price}`));
  ...
  const firstExperienceId = itemsRow.rows.find((r: any) => r.item_type === "experience")?.ref_id;
